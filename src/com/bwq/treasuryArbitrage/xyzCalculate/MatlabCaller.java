...
public class MatlabCaller {
	 ...
	 */
	public Object[] Arbitrage_Main(ArrayList<Double>  Lf1,ArrayList<Double>  Lf2,
			double stop_loss,double stop_profit,double tradeCost){
		Object[] result = null;
		MWNumericArray f1 = null,f2 = null,mwSL = null,mwSP = null,mwtradeCost = null;   
		...
		mwtradeCost = new MWNumericArray(tradeCost,MWClassID.DOUBLE);
		try {
			//Arbitrage_Main方法调用示例
			//第一个参数数字表示需要获得的结果个数，比如这里的3，表示获得三个结果，x,y,k;如果写2,则只能得到x,y;
			//下同
			result =mFXY.Arbitrage_Main(3,f1,f2,mwSL, mwSP,mwtradeCost);

...
public class OptimalKTCalculator implements Runnable {
	...
	public void run() {
			...
			System.out.println("KT Construction Done...");
			while (true) {
				now = new Date();
				...
				if (!todayRun && now_hour >= 0 && now_min >= 2) {
					todayRun = true;
					sleepPF = true;
					System.out.println("KT Thread run...");
					...
					for (int p = 1; p < 4; p++) // p������
					{
						...
						if (lf1.size() != lf2.size()) {
							...
							result = dm.Arbitrage_Main2(plf1, plf2, stop_loss,
									stop_profit);
							System.out.println("Calculate Done...");
							// result = dm.Open(lf1, lf2, 91, 92,0,0,93); test
							if (result != null) {
								double OptimalK,OptimalT;
								OptimalK = Double.valueOf(String.valueOf(result[0]));
